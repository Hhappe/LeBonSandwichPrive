...
class lbscontrol
{
	...
	public function toutesCommandes(Request $req, Response $resp, $args)
	{
		...
		return (new \lbs\view\lbsview($tab))->render('toutesCommandes', $req, $resp, $args);
    }
		...
		if(\lbs\model\sandwich::where('id', $id)->get()->toJson() != "[]")
		{
			...
			if(($obj->etat < 6) && ($obj->etat > 0))
			{
				if($commande->etat > $obj->etat)
				{
					...
					return (new \lbs\view\lbsview($arr))->render('changementEtat', $req, $resp, $args);
				}
				else
				{
					$commande->etat = $obj->etat
					$commande->save();
					return (new \lbs\view\lbsview($commande))->render('changementEtat', $req, $resp, $args);
				}
			}
			else
			{
